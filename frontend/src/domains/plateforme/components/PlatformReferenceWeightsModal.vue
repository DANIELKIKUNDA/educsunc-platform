<template>
  <PlatformReferenceModal
    id="reference-weights-modal"
    :open="open"
    eyebrow="Ponderations"
    title="Modifier les ponderations"
    description="Ajustez les maxima de la ligne selectionnee sans toucher aux autres attributs."
    @close="$emit('close')"
  >
    <div class="reference-import__grid">
      <label
        v-for="field in ponderationFields"
        :key="field.key"
        class="reference-center__field"
      >
        <span>{{ field.label }}</span>
        <input v-model.number="form[field.key]" type="number" min="0" />
      </label>
    </div>

    <template #footer>
      <button class="reference-center__ghost-button" type="button" @click="$emit('close')">
        Annuler
      </button>
      <button class="reference-center__primary-button" type="button" @click="submit">
        Enregistrer les ponderations
      </button>
    </template>
  </PlatformReferenceModal>
</template>

<script setup lang="ts">
import { reactive, watch } from 'vue';
import type { LigneReferentielProgrammeItem } from '../../academique/models/academique.model';
import PlatformReferenceModal from './PlatformReferenceModal.vue';

type PonderationKey =
  | 'maxP1'
  | 'maxP2'
  | 'maxEX1'
  | 'maxP3'
  | 'maxP4'
  | 'maxEX2'
  | 'maxP5'
  | 'maxP6'
  | 'maxEX3';

const props = defineProps<{
  open: boolean;
  line: LigneReferentielProgrammeItem | null;
}>();

const emit = defineEmits<{
  close: [];
  submit: [payload: { ponderation: Record<string, number> }];
}>();

const ponderationFields: Array<{ key: PonderationKey; label: string }> = [
  { key: 'maxP1', label: 'P1' },
  { key: 'maxP2', label: 'P2' },
  { key: 'maxEX1', label: 'EX1' },
  { key: 'maxP3', label: 'P3' },
  { key: 'maxP4', label: 'P4' },
  { key: 'maxEX2', label: 'EX2' },
  { key: 'maxP5', label: 'P5' },
  { key: 'maxP6', label: 'P6' },
  { key: 'maxEX3', label: 'EX3' },
];

const form = reactive(createBlankPonderation());

watch(
  () => [props.open, props.line] as const,
  ([open, line]) => {
    if (!open) {
      return;
    }

    Object.assign(form, createBlankPonderation(), line?.ponderation ?? {});
  },
  { immediate: true },
);

function createBlankPonderation(): Record<PonderationKey, number> {
  return {
    maxP1: 0,
    maxP2: 0,
    maxEX1: 0,
    maxP3: 0,
    maxP4: 0,
    maxEX2: 0,
    maxP5: 0,
    maxP6: 0,
    maxEX3: 0,
  };
}

function submit(): void {
  emit('submit', { ponderation: { ...form } });
}
</script>
