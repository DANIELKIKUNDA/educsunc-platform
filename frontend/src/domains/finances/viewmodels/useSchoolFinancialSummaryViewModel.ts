...
import type { SchoolFinancialSummaryFilters } from '../models/school-financial-summary.model';
import { useSchoolFinancialSummaryStore } from '../stores/school-financial-summary.store';
import {
...
export function useSchoolFinancialSummaryViewModel() {
  const context = activeContextStore.state;
  const session = sessionStore.state;
  const route = useRoute();
  const router = useRouter();
  const summaryStore = useSchoolFinancialSummaryStore();
  const doctrineAccess = useDoctrineAccess();

  const idAnneeScolaireInput = ref('');
  const anneeScolaireLabelInput = ref('');
  const ecoleLabelInput = ref('');
  ...
  const isAuthorized = computed(() => doctrineAccess.canAccessPage('VF-04'));
  const summary = computed(() => summaryStore.state.summary);
  const technicalErrorMessage = computed(() =>
    summaryStore.state.errorMessage ?? 'Le backend n a pas pu restituer la synthese financiere d ecole.',
  );
  ...
  const fallbackScopeLabel = computed(() => {
    const ecole = ecoleLabelInput.value.trim() || context.schoolName;
  ...
  const perimeterMessage = computed(() => {
    switch (session.actorCode) {
      ...
      case 'GESTIONNAIRE_ORGANISATION':
        return `Lecture bornee a l organisation active ${context.organizationName}, avec descente controlee vers l ecole active.`;
      default:
  ...
  function synchroniserDepuisRoute(): void {
    ...
    ecoleLabelInput.value = lireQueryString('ecole') || context.schoolName;
    moisAnalyseInput.value = lireQueryString('moisAnalyseJusqua');
    typeFraisInput.value = lireQueryString('typeFrais');
  }

  function construireFiltres(): SchoolFinancialSummaryFilters {
    return {
      ...
      ecoleLabel: ecoleLabelInput.value.trim() || undefined,
    };
  }
  ...
  function syntheseSectionLink(idSectionScolaire: string, section: string) {
    return {
  ...
  function syntheseClasseHintLink(idSectionScolaire: string, section: string) {
    return {
  ...
  function registreClasseHintLink(idSectionScolaire: string, section: string) {
    return {
  ...
  function exporterCsv(): void {
    ...
    const headers = ['Section', 'Effectif total', 'Redevables', 'En ordre', 'Non en ordre', 'Montant attendu', 'Montant recouvre', 'Reste', 'Taux'];
    const lines = [...summary.value.rows, summary.value.totalGeneralEcole].map((row) => [
      row.section,
    ...
    link.href = url;
    link.download = `synthese-financiere-ecole-${idAnneeScolaireInput.value || 'ecole'}.csv`;
    link.click();
  ...
  function construireHtmlImprimable(): string {
    if (!summary.value) return '';
    const bodyRows = [...summary.value.rows, summary.value.totalGeneralEcole]
      .map((row) => `
      <tr>
        <td>${row.section}</td>
    ...
    <html lang="fr">
      <head>
        <meta charset="utf-8" />
        <title>Synthese financiere ecole</title>
        <style>
      ...
      <body>
        <h1>Situation financiere synthetique ecole</h1>
