          ...
          >
            <div class="finance-kpi-grid finance-kpi-grid--detail">
              ...
              <div class="finance-kpi-card">
                ...
                <span>Restes lies aux periodes anterieures</span>
              </div>
            </div>
...
import {
  authorizedStudentFinancialSituationActors,
  studentFinancialSituationProfile,
} from '../data/situation-financiere-eleve.demo';

const context = activeContextStore.state;
const session = sessionStore.state;
const profile = ref({ ...studentFinancialSituationProfile });
...
const isAuthorized = computed(() =>
  authorizedStudentFinancialSituationActors.includes(session.actorCode as never),
);
...
const availableTypes = computed(() => [...new Set(profile.value.obligations.map((item) => item.typeFrais))]);
const availableStatuses = computed(() => [...new Set(profile.value.obligations.map((item) => item.statut))]);

const filteredObligations = computed(() =>
  profile.value.obligations.filter((item) => {
    ...
    const matchesSegment =
      ...
      (selectedSegment.value === 'EXIGIBLE' && item.reste > 0) ||
      (selectedSegment.value === 'ARRIERE' && item.reste > 0 && item.periode !== 'Juin 2026');
...
const exigibleObligations = computed(() => profile.value.obligations.filter((item) => item.reste > 0));

const alertClass = computed(() =>
  profile.value.totalDette > 0 ? 'finance-status-strip--error' : 'finance-status-strip--success',
);

const alertTitle = computed(() =>
  profile.value.totalDette > 0 ? 'Situation avec dette restante' : 'Situation financiere en ordre',
);

const alertMessage = computed(() =>
  profile.value.totalDette > 0
