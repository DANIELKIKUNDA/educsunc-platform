...
test('SECURITY applique bien les regles d acces sur le referentiel', async () => {
  const bootstrap = new GlobalTestBootstrap();
  const admin = await bootstrap.creerActeur({ ...ROLE_FIXTURES.ADMIN_SYSTEME_ECOLE, organisationId: TENANT_FIXTURES.organisationA, ecoleId: TENANT_FIXTURES.ecoleA1 });
  ...
  assert.equal((await injecterCommeActeur(serveur, enseignant, { method: 'GET', url: '/bc/referentiel/lire' })).statusCode, 200);
  assert.equal((await injecterCommeActeur(serveur, parent, { method: 'POST', url: '/bc/referentiel/modifier' })).statusCode, 403);
  assert.equal((await injecterCommeActeur(serveur, prefet, { method: 'GET', url: '/bc/referentiel/lire' })).statusCode, 200);
