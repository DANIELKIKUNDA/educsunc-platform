...
test('SECURITY reserve PLT-03 a MANAGER_SYSTEME et autorise OPERATEUR_SYSTEME seulement si la delegation explicite est active', async () => {
  ...
  const adaptateurSansDelegation = new AutorisationImportReferentielAdapter({
  ...
  const adaptateurAvecDelegation = new AutorisationImportReferentielAdapter({
    roleRepository: bootstrap.securityRepositories.roleRepository,
    affectationRepository: bootstrap.securityRepositories.affectationRepository,
    securityFacade: bootstrap.securityFacade,
  ...
  await adaptateurSansDelegation.verifierMutationImportReferentiel({
    idUtilisateur: manager.utilisateurId,
    roleActif: manager.roleCode,
  });

  await assert.rejects(
    () => adaptateurSansDelegation.verifierMutationImportReferentiel({
      idUtilisateur: operateur.utilisateurId,
      roleActif: operateur.roleCode,
    }),
    /importer une version officielle du referentiel|autorise/i,
  );

  await adaptateurAvecDelegation.verifierMutationImportReferentiel({
    idUtilisateur: operateur.utilisateurId,
    roleActif: operateur.roleCode,
  });

  await assert.rejects(
    () => adaptateurAvecDelegation.verifierMutationImportReferentiel({
