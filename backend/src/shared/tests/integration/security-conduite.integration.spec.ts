...
import { AutorisationConduiteAdapter } from '../../../app/adapters/AutorisationConduiteAdapter';
...
test('SECURITY applique bien les regles locales d encodage de la conduite', async () => {
  ...
  const adaptateur = new AutorisationConduiteAdapter({
  ...
  await adaptateur.verifierEncodageConduite({
  ...
  await assert.rejects(
    () => adaptateur.verifierEncodageConduite({
    ...
    /pas autorise|permission|refuse/i,
  );

  await assert.rejects(
    () => adaptateur.verifierEncodageConduite({
      idUtilisateur: administrateurEcole.utilisateurId,
      idOrganisation: TENANT_FIXTURES.organisationA,
      idEcole: TENANT_FIXTURES.ecoleA1,
      idClassePedagogique: WORKFLOW_FIXTURES.classeA,
      idAnneeScolaire: WORKFLOW_FIXTURES.anneeScolaireId,
