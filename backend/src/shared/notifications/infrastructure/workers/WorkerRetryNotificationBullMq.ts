// Ce fichier declare le worker de retry BullMQ du moteur Notifications.

import { randomUUID } from 'node:crypto';
import { PortMonitoringNotification } from '../../application';
import { FileRetryNotificationsBullMq, JobFileNotification } from '../queues';
import { EntreeRetryNotification, ResultatExecutionRetryNotification } from '../retry/TypesRetryNotification';
import { ResultatExecutionWorkerNotification } from './TypesWorkersNotifications';

/** Cette classe consomme la file BullMQ de retry et conserve un historique technique local. */
export class WorkerRetryNotificationBullMq {
  private readonly historiquesParNotification = new Map<string, EntreeRetryNotification[]>();

  /** Ce constructeur assemble les dependances techniques du retry BullMQ. */
  constructor(
  ...
  /** Cette methode execute un cycle complet de retry BullMQ. */
  public async executerCycle(limite = 10): Promise<ResultatExecutionWorkerNotification> {
    ...
    while (resultats.length < limite) {
      const job = await this.fileRetryNotificationsBullMq.extraireProchainDisponible();
      if (!job) {
        break;
      }
  ...
  /** Cette methode retourne l historique local d une notification en retry. */
  ...
  private async executerJob(job: JobFileNotification): Promise<ResultatExecutionRetryNotification> {
    ...
    try {
      ...
      return {
        ...
        succes: true,
        message: 'Retry technique BullMQ execute.',
        horodatage: new Date(),
      };
    } catch (erreur) {
      const message = erreur instanceof Error ? erreur.message : 'Echec technique de retry BullMQ.';
