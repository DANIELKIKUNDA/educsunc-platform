...
test('generation refresh token', () => {
  const resultat = moteur.generer('utilisateur-1', 'session-1', 1);
...
test('rotation refresh token', () => {
  const ancien = moteur.generer('utilisateur-1', 'session-1', 1).refreshToken;
  moteur.revoquer(ancien);
  const nouveau = moteur.generer('utilisateur-1', 'session-1', 1).refreshToken;
  assert.notEqual(ancien.obtenirId(), nouveau.obtenirId());
});

test('revocation refresh token', () => {
  const refreshToken = moteur.generer('utilisateur-1', 'session-1', 1).refreshToken;
