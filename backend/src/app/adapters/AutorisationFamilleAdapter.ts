import type { AutorisationFamillePort } from '../../contexts/scolarite-eleves/application/ports';
...
// Cet adaptateur reapplique shared/security pour le workflow familles porte par le caissier.
export class AutorisationFamilleAdapter implements AutorisationFamillePort {
  ...
  public async verifierLectureFamille(params: {
  ...
  public async verifierMutationFamille(params: {
    idUtilisateur: string;
    idOrganisation: string;
    idEcole: string;
  }): Promise<void> {
  ...
  private async verifierAccesCaissier(
    ...
    throw new ErreurAutorisation(
      "Seul un caissier de l'ecole courante peut gerer ou consulter les familles.",
    );
  }
}
