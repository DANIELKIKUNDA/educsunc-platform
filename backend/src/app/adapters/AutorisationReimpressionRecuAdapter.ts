...
// Cette classe limite la reimpression des recus au seul caissier de l'ecole courante.
export class AutorisationReimpressionRecuAdapter
  implements AutorisationReimpressionRecuPort
{
  ...
  public async verifierReimpressionRecu(params: {
    ...
    if (!estCaissier) {
      throw new ErreurDroitsInsuffisants(
        "Seul un caissier actif de l'ecole peut reimprimer un recu.",
      );
    }
