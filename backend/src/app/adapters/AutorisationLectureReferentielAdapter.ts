...
const ROLE_PRINCIPAL_LECTURE_REFERENTIEL = 'MANAGER_SYSTEME';
const ROLE_SECONDAIRE_LECTURE_REFERENTIEL = 'OPERATEUR_SYSTEME';

interface DependancesAutorisationLectureReferentielAdapter {
...
// Cet adaptateur verrouille PLT-05 sur les acteurs systeme reels et une delegation explicite d'OPERATEUR_SYSTEME.
export class AutorisationLectureReferentielAdapter {
  ...
  constructor(dependances?: DependancesAutorisationLectureReferentielAdapter) {
    ...
    this.autoriserOperateur =
      ...
      ?? configurationApplication.autoriserOperateurWorkflowLectureReferentiel;
  }

  public async verifierLectureReferentiel(params: {
    idUtilisateur: string;
    roleActif?: string;
  }): Promise<void> {
    await this.verifierRoleLectureReferentielAutorise(params);
  ...
  private async verifierRoleLectureReferentielAutorise(params: {
    idUtilisateur: string;
    roleActif?: string;
  }): Promise<void> {
    ...
    throw new Error("L'acteur courant n'est pas autorise a consulter les referentiels officiels.");
  }

  private roleAutorise(roleActif: string): boolean {
    if (roleActif === ROLE_PRINCIPAL_LECTURE_REFERENTIEL) {
      return true;
    }

    return this.autoriserOperateur && roleActif === ROLE_SECONDAIRE_LECTURE_REFERENTIEL;
  }
}
