    ...
    },
    {
      lister: async (idOrganisation: string) => {
        ...
        const resultat = await infrastructure.clientLecture.executer<LigneHistoriqueOrganisation>(
          [
            'WITH historique AS (',
            ' SELECT "id", "action", "acteur", "details", "cree_le"',
            ' FROM "audit_logs"',
            ' WHERE "id_organisation" = $1',
            '    OR ("type_ressource" = $2 AND "id_ressource" = $1::text)',
            ' UNION ALL',
            ' SELECT id_audit_entry AS "id",',
            '   COALESCE(metadata->>\'actionSource\', action) AS "action",',
            '   acteur_id AS "acteur", metadata AS "details", date_action AS "cree_le"',
            ' FROM audit_entries',
            ' WHERE organisation_id = $1',
            '   AND (type_ressource = $2 OR id_ressource = $1::text)',
            ')',
            'SELECT',
            '  "id", "action", "acteur", "details", "cree_le",',
            '  CASE',
